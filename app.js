...
(function () {
  ...
  // ──────────────────────────────────────────────────────────────
  // §5b  AUDIO — Alertas sonoras (Web Audio API, sin dependencias)
  // ──────────────────────────────────────────────────────────────
  let _audioCtx = null;
  function _getAudio() {
  ...
  /**
   * Emite un tono suave de confirmación.
   * @param {'new'|'pay'|'alert'} type
   */
  function _beep(type = 'new') {
    try {
      const ctx  = _getAudio();
      const osc  = ctx.createOscillator();
      const gain = ctx.createGain();
      osc.connect(gain); gain.connect(ctx.destination);
      if (type === 'new')   { osc.frequency.value = 880; }
      if (type === 'pay')   { osc.frequency.value = 1047; }
      if (type === 'alert') { osc.frequency.value = 440;  }
      ...
      if (type === 'pay') {
        // Doble tono para pago: nota extra medio segundo después
        const osc2  = ctx.createOscillator();
        const gain2 = ctx.createGain();
        osc2.connect(gain2); gain2.connect(ctx.destination);
        osc2.frequency.value = 1319;
        gain2.gain.setValueAtTime(.2, ctx.currentTime + .28);
        gain2.gain.exponentialRampToValueAtTime(.001, ctx.currentTime + .5);
        osc2.start(ctx.currentTime + .28);
        osc2.stop(ctx.currentTime + .5);
      }
    } catch(e) { /* Audio no disponible en este contexto */ }
  }

  // ──────────────────────────────────────────────────────────────
  // §5c  ESTADO DE CONEXIÓN
  // ──────────────────────────────────────────────────────────────
  function _updateOnlineUI(online) {
    const dot   = document.getElementById('onlineDot');
    const label = document.getElementById('onlineLabel');
    if (!dot || !label) return;
    if (online) {
      dot.className   = 'w-2 h-2 rounded-full bg-emerald-400 shadow-[0_0_6px_#4ade80]';
      label.textContent = 'En Línea';
      label.className = 'text-nano font-black uppercase text-emerald-600 tracking-wider';
    } else {
      dot.className   = 'w-2 h-2 rounded-full bg-red-400 animate-pulse';
      label.textContent = 'Sin Conexión';
      label.className = 'text-nano font-black uppercase text-red-500 tracking-wider';
    }
  }
  window.addEventListener('online',  () => { _updateOnlineUI(true);  _pub._toast('Conexión restaurada ✓', '#16a34a'); });
  window.addEventListener('offline', () => { _updateOnlineUI(false); _pub._toast('Sin conexión a internet', '#ef4444'); });
  ...
  const _pub = {
    ...
    _syncFromFirestore() {
      ...
      if (rol === 'admin')   {
        ...
        _pub.renderDashboard();
      }
      else if (rol === 'cajero')  _pub.renderCajero();
      else if (rol === 'barbero') _pub.renderBarbero();
    },

    // ── Dashboard KPI (barra superior del panel principal) ────
    renderDashboard() {
      const hoy      = (_data.hoy || []).filter(v => !v.type);
      const activos  = (_data.activos || []).length;
      const cola     = (_data.espera  || []).length;
      const ingresos = hoy.reduce((a, v) => a + (v.price || 0), 0);
      _set('dkpiHoy',      String(hoy.length));
      _set('dkpiIngresos', fmt(ingresos));
      _set('dkpiActivos',  String(activos));
      _set('dkpiCola',     String(cola));
    },
    ...
    regCajero() {
      ...
      if (!asignado) {
        ...
        _beep('new'); _pub._toast('Todos ocupados — cliente en cola', '#f59e0b');
      }
    ...
    reg() {
      ...
      if (!asignado) {
        _data.espera.push(cliente);
        _beep('new'); _pub._toast('Todos ocupados — cliente en cola de espera', '#f59e0b');
      }
    ...
    _statsGetVentas(periodo) {
      ...
      let desde = 0, hasta = Date.now();

      if (periodo === 'rango') {
        const d = document.getElementById('statsDesde')?.value;
        const h = document.getElementById('statsHasta')?.value;
        if (!d || !h) return [];
        desde = new Date(d + 'T00:00:00').getTime();
        hasta = new Date(h + 'T23:59:59').getTime();
      } else if (periodo === 'todo') {
        desde = 0; hasta = Date.now();
      } else {
        const ms = { hoy: 86400000, semana: 7*86400000, mes: 30*86400000,
                     trimestre: 90*86400000, año: 365*86400000 };
        desde = Date.now() - (ms[periodo] || ms.hoy);
      }

      // Ventas del día actual (aún sin cerrar)
      const hoyVentas = (_data.hoy || []).filter(v => {
        if (v.type) return false;
        const t = new Date(v.timestamp).getTime();
        return t >= desde && t <= hasta;
      });

      // Ventas de cierres históricos dentro del rango
      const histVentas = [];
      (_data.historial || []).forEach(c => {
        (c.detalle || []).forEach(v => {
          if (v.type) return;
          const t = new Date(v.timestamp).getTime();
          if (t >= desde && t <= hasta) histVentas.push(v);
        });
      });
    ...
    renderEstadisticas(periodo) {
      ...
      const labels = {
        ...
        trimestre: 'Últimos 90 días', año: 'Último año', todo: 'Todo el historial',
        rango: (() => {
          const d = document.getElementById('statsDesde')?.value;
          const h = document.getElementById('statsHasta')?.value;
          return d && h ? `${d} → ${h}` : 'Rango personalizado';
        })(),
      };
      ...
      const _mkCierreSearch = c => `${c.fecha||''} ${(c.perBarber||[]).map(b=>b.nombre).join(' ')} ${c.sis?.tot||0}`.toLowerCase();
    ...
    filtrarStats(periodo, btn) {
      ...
      if (btn) btn.classList.add('active');
      // Mostrar/ocultar el selector de rango personalizado
      const rangoRow = document.getElementById('statsRangoRow');
      if (rangoRow) rangoRow.classList.toggle('hidden', periodo !== 'rango');
      if (periodo !== 'rango') _pub.renderEstadisticas(periodo);
      // Si es rango, el usuario debe pulsar "Aplicar" para filtrar
    },
    ...
    aplicarRangoStats() {
      const d = document.getElementById('statsDesde')?.value;
      const h = document.getElementById('statsHasta')?.value;
      if (!d || !h) { _pub._toast('Selecciona fecha de inicio y fin', '#f59e0b'); return; }
      if (d > h)    { _pub._toast('La fecha inicial debe ser anterior a la final', '#ef4444'); return; }
      _pub.renderEstadisticas('rango');
    },

    // Exporta los datos del período actual como CSV
    exportarStats() {
      const rangoRow = document.getElementById('statsRangoRow');
      const esPeriodoRango = rangoRow && !rangoRow.classList.contains('hidden');
      const periodo = esPeriodoRango ? 'rango' : 'mes';
      const ventas  = _pub._statsGetVentas(periodo);
      if (!ventas.length) { _pub._toast('Sin datos para exportar', '#f59e0b'); return; }

      const enc  = ['Fecha', 'Cliente', 'Servicio', 'Barbero', 'Precio', 'Método'];
      const rows = ventas.map(v => [
        v.timestamp ? new Date(v.timestamp).toLocaleString('es-CL') : '',
        v.name    || '',
        v.service || '',
        v.barber  || '',
        v.price   || 0,
      ...
      const csv  = [enc, ...rows].map(r => r.map(c => `"${String(c).replace(/"/g,'""')}"`).join(',')).join('\n');
      const blob = new Blob([csv], { type: 'text/csv;charset=utf-8;' });
      const url  = URL.createObjectURL(blob);
      const a    = document.createElement('a');
      a.href = url; a.download = `fabiola_stats_${new Date().toISOString().slice(0,10)}.csv`;
      a.click(); URL.revokeObjectURL(url);
      _pub._toast('CSV descargado ✓', '#16a34a');
    },

    // Búsqueda en historial de cierres
    buscarEnCierres(query) {
      const q = query.toLowerCase().trim();
      const lista = document.getElementById('statCierreList');
      if (!lista) return;
      const items = lista.querySelectorAll('[data-cierre]');
      items.forEach(el => {
        const txt = el.dataset.cierre || '';
  ...
  window.app = Object.freeze({
    ...
    renderDashboard:     ()    => _pub.renderDashboard(),
    aplicarRangoStats:   ()    => _pub.aplicarRangoStats(),
    exportarStats:       ()    => _pub.exportarStats(),
    buscarEnCierres:     q     => _pub.buscarEnCierres(q),
