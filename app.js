...
(function () {
  ...
  const _pub = {
    ...
    // ──── §10.14 IMPRESIÓN ────────────────────────────────────
    _css: `body{font-family:'Courier New',monospace;width:72mm;margin:0;padding:5mm;font-size:11px;line-height:1.35;color:#000;}
      .c{text-align:center;}.b{font-weight:bold;}.hr{border:none;border-bottom:1px dashed #000;margin:5px 0;}
      .f{display:flex;justify-content:space-between;}.big{font-size:13px;}.sm{font-size:9px;}@page{margin:0;}`,

    imprimirTicketVenta(v) {
      const w = window.open('','_blank','width=300');
      w.document.write(`<html><head><style>${_pub._css}</style></head><body>
        <div class="c b big">FABIOLA</div>
        <div class="c">Peluquería & Estética</div>
        <div class="hr"></div>
        <div class="f"><span>FECHA:</span><span>${new Date().toLocaleDateString('es-CL')}</span></div>
        <div class="f"><span>PROFESIONAL:</span><span>${v.barber}</span></div>
        <div class="hr"></div>
        <div class="c b">${v.name}</div>
        <div class="c b big" style="margin:10px 0;">${fmt(v.price)}</div>
        <div class="hr"></div>
        <div class="f"><span>MÉTODO DE PAGO:</span><span>${v.method}</span></div>
        <div class="hr"></div>
        <div class="c sm">GRACIAS POR TU PREFERENCIA</div>
    ...
    imprimirPagoStaff(nombre, cant, monto, f1, f2) {
      ...
      w.document.write(`<html><head><style>${_pub._css}</style></head><body>
        ...
        <div style="border-top:1px solid #000;text-align:center;padding-top:5px;">FIRMA DE CONFORMIDAD</div>
        <div class="c sm" style="margin-top:8px;">San Martin 443, Castro</div>
      </body></html>`);
      w.document.close(); w.print(); w.close();
    },
