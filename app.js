...
(function () {
  ...
  let _ventasChart = null; // Instancia Chart.js para estadísticas
  ...
  function _buildMonitorHTML() {
    ...
    // ── Columna 1: Fila de barberos ──────────────────────────
    const barberoRows = turnos.map(bName => {
      ...
      const statusColor = isProx ? '#14b8a6' : isBusy ? '#ef4444' : isPausa ? '#f59e0b' : '#737373';
      const statusText  = isProx ? 'Siguiente' : isBusy ? 'Atendiendo' : isPausa ? 'Pausa' : 'Disponible';
      const dotAnim     = isProx ? 'animation:pulse 1.4s infinite;' : isBusy ? 'animation:pulse 1.8s infinite;' : '';

      const photoEl = b.photo
        ? `<img src="${b.photo}" style="width:40px;height:40px;border-radius:10px;object-fit:cover;border:2px solid ${statusColor};flex-shrink:0;">`
        : `<div style="width:40px;height:40px;border-radius:10px;background:${statusColor}22;border:2px solid ${statusColor};color:${statusColor};font-size:1rem;font-weight:900;display:flex;align-items:center;justify-content:center;flex-shrink:0;">${bName.charAt(0)}</div>`;

      const actionIcon = isBusy
        ? `<span style="color:#ef4444;font-size:11px;">✂</span>`
        : isProx
        ? `<span style="color:#14b8a6;font-size:11px;">●</span>`
        : `<span style="color:${statusColor};font-size:11px;">○</span>`;

      return `
        <div style="display:flex;align-items:center;gap:12px;padding:10px 14px;background:rgba(255,255,255,.04);border-radius:12px;border:1px solid rgba(255,255,255,.06);margin-bottom:8px;${isProx ? 'border-color:rgba(20,184,166,.4);background:rgba(20,184,166,.07);' : ''}">
          ${photoEl}
          <div style="flex:1;min-width:0;">
            <div style="font-size:13px;font-weight:900;color:#f5f5f5;text-transform:uppercase;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;">${bName}</div>
            ${isBusy && cliente ? `<div style="font-size:10px;color:#737373;font-weight:700;margin-top:1px;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;">→ ${cliente.name}</div>` : ''}
          </div>
          <div style="display:flex;align-items:center;gap:5px;flex-shrink:0;">
            <span style="width:6px;height:6px;border-radius:50%;background:${statusColor};display:inline-block;${dotAnim}"></span>
            <span style="font-size:9px;font-weight:900;color:${statusColor};text-transform:uppercase;letter-spacing:.04em;">${statusText}</span>
            ${actionIcon}
          </div>
        </div>`;
    }).join('') || `<div style="text-align:center;padding:32px 12px;color:#404040;font-size:10px;font-weight:700;text-transform:uppercase;">Sin profesionales en turno</div>`;

    // ── Columna 2: Atendiéndose ahora ────────────────────────
    const atendiendoRows = activos.length
      ? activos.map(a => {
          const elapsed = Math.floor((Date.now() - a.startTime) / 60000);
          return `
            <div style="padding:14px 16px;background:rgba(255,255,255,.04);border-radius:12px;border:1px solid rgba(255,255,255,.06);margin-bottom:10px;">
              <div style="font-size:9px;color:#737373;font-weight:900;text-transform:uppercase;letter-spacing:.08em;margin-bottom:4px;">
                <span style="color:#14b8a6;">▪</span> Cliente ${activos.indexOf(a)+1}:
              </div>
              <div style="font-size:15px;font-weight:900;color:#f5f5f5;margin-bottom:3px;">${a.name}</div>
              <div style="font-size:10px;color:#737373;font-weight:700;">con Barbero <strong style="color:#f5f5f5;">${a.barber.toUpperCase()}</strong></div>
              <div style="display:flex;align-items:center;gap:6px;margin-top:8px;">
                <span style="font-size:11px;">⏱</span>
                <span style="font-size:10px;font-weight:900;color:#14b8a6;">${elapsed} min</span>
              </div>
            </div>`;
        }).join('')
      : `<div style="text-align:center;padding:40px 12px;color:#404040;font-size:10px;font-weight:700;text-transform:uppercase;">Sin clientes en atención</div>`;

    // ── Columna 3: Lista de espera ────────────────────────────
    const esperaRows = espera.length
      ? espera.map((c, i) => {
          const isNext = i === 0;
          return `
            <div style="display:flex;align-items:center;gap:12px;padding:${isNext ? '14px 16px' : '10px 14px'};background:${isNext ? 'rgba(20,184,166,.12)' : 'rgba(255,255,255,.03)'};border-radius:${isNext ? '12px' : '10px'};border:${isNext ? '1.5px solid rgba(20,184,166,.5)' : '1px solid rgba(255,255,255,.05)'};margin-bottom:${isNext ? '10px' : '6px'};">
              <div style="width:${isNext ? '38px' : '30px'};height:${isNext ? '38px' : '30px'};border-radius:8px;background:${isNext ? '#14b8a6' : 'rgba(255,255,255,.07)'};color:${isNext ? '#0a0a0a' : '#737373'};font-size:${isNext ? '15px' : '12px'};font-weight:900;display:flex;align-items:center;justify-content:center;flex-shrink:0;">${i+1}</div>
              <div style="flex:1;min-width:0;">
                <div style="font-size:${isNext ? '15px' : '13px'};font-weight:900;color:${isNext ? '#f5f5f5' : '#a3a3a3'};text-transform:uppercase;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;">${c.name}</div>
                ${isNext ? `<div style="font-size:9px;color:#14b8a6;font-weight:700;margin-top:2px;text-transform:uppercase;letter-spacing:.04em;">Próximo Cliente 🔔</div>` : ''}
              </div>
              <div style="font-family:monospace;font-size:${isNext ? '20px' : '14px'};font-weight:900;color:${isNext ? '#14b8a6' : '#525252'};flex-shrink:0;">${String(i + 1).padStart(3, '0')}</div>
            </div>`;
        }).join('')
      : `<div style="text-align:center;padding:40px 12px;color:#404040;font-size:10px;font-weight:700;text-transform:uppercase;">Sin clientes en espera</div>`;

    return `<!DOCTYPE html><html lang="es"><head>
      <meta charset="UTF-8"><meta name="viewport" content="width=device-width,initial-scale=1">
      <title>Fabiola · Sala de Espera</title>
      <link rel="preconnect" href="https://fonts.googleapis.com">
      <link href="https://fonts.googleapis.com/css2?family=DM+Sans:wght@400;700;900&family=JetBrains+Mono:wght@700;800&display=swap" rel="stylesheet">
      <style>
        *{margin:0;padding:0;box-sizing:border-box;}
        body{
          ...
          background:#080f0d;
          background-image:
            radial-gradient(ellipse 60% 50% at 10% 90%, rgba(20,184,166,.08) 0%, transparent 60%),
            radial-gradient(ellipse 40% 40% at 90% 10%, rgba(20,184,166,.06) 0%, transparent 60%),
            linear-gradient(rgba(20,184,166,.02) 1px, transparent 1px),
            linear-gradient(90deg, rgba(20,184,166,.02) 1px, transparent 1px);
        ...
        .col-label{font-size:9px;font-weight:900;letter-spacing:.2em;text-transform:uppercase;padding:7px 14px;margin-bottom:14px;display:inline-flex;align-items:center;gap:8px;}
        .grid-3{display:grid;grid-template-columns:240px 1fr 300px;gap:0;height:calc(100vh - 100px);}
        .col{padding:20px 18px;overflow:hidden;}
        .col-divider{border-right:1px solid rgba(255,255,255,.06);}
        .col-header{padding:8px 18px 14px;border-bottom:1px solid rgba(255,255,255,.06);margin-bottom:16px;}
        .col-header-teal{background:rgba(20,184,166,.15);border-bottom:1px solid rgba(20,184,166,.25);}
        .monofont{font-family:'JetBrains Mono',monospace;}
      </style>
    </head><body>

      <!-- TOP BAR -->
      <div style="display:flex;justify-content:space-between;align-items:center;padding:14px 28px;border-bottom:1px solid rgba(255,255,255,.06);background:rgba(0,0,0,.3);backdrop-filter:blur(10px);">
        <div style="display:flex;align-items:center;gap:12px;">
          <div style="width:32px;height:32px;background:#14b8a6;border-radius:8px;display:flex;align-items:center;justify-content:center;font-size:16px;">✂</div>
          <div>
            <div style="font-size:18px;font-weight:900;letter-spacing:-.02em;line-height:1;">FABIOLA<span style="color:#14b8a6;">.</span></div>
            <div style="font-size:8px;font-weight:700;color:#737373;letter-spacing:.2em;text-transform:uppercase;">Peluquería &amp; Estética · San Martín 443, Castro</div>
          </div>
        </div>
        <div style="text-align:right;">
          <div id="monClock" class="monofont" style="font-size:22px;font-weight:800;color:#14b8a6;">${hora}</div>
          <div style="font-size:9px;color:#525252;font-weight:700;text-transform:capitalize;margin-top:1px;">${fecha}</div>
        </div>
        <div style="display:flex;align-items:center;gap:8px;">
          <div style="width:6px;height:6px;border-radius:50%;background:#14b8a6;animation:pulse 2s infinite;"></div>
          <span style="font-size:9px;font-weight:900;color:#14b8a6;text-transform:uppercase;letter-spacing:.1em;">EN VIVO</span>
        </div>
      </div>
      ...
      <div class="grid-3">
        ...
        <!-- Col 1: Barberos en Turno -->
        <div class="col col-divider" style="background:rgba(0,0,0,.15);">
          <div class="col-header">
            <div class="col-label" style="color:#f5f5f5;">
        ...
        <!-- Col 2: Atendiéndose Ahora -->
        <div class="col col-divider" style="background:rgba(20,184,166,.02);">
          <div class="col-header col-header-teal">
            <div class="col-label" style="color:#14b8a6;">
        ...
        <!-- Col 3: Lista de Espera -->
        <div class="col" style="background:rgba(0,0,0,.1);">
          <div class="col-header" style="display:flex;justify-content:space-between;align-items:center;padding-bottom:12px;border-bottom:1px solid rgba(255,255,255,.06);margin-bottom:14px;">
            <div class="col-label" style="color:#f5f5f5;padding:0;">
              <span>LISTA DE ESPERA</span>
            </div>
            <div style="display:flex;flex-direction:column;text-align:right;line-height:1.2;">
              <span style="font-size:8px;color:#737373;font-weight:700;text-transform:uppercase;">Turno</span>
              <span style="font-size:8px;color:#737373;font-weight:700;">Próximo Cliente 🔔</span>
            </div>
          </div>
          <div style="overflow-y:auto;height:calc(100% - 70px);">
            ${esperaRows}
      ...
      <!-- TICKER BOTTOM -->
      <div style="position:fixed;bottom:0;left:0;right:0;height:28px;background:#14b8a6;display:flex;align-items:center;overflow:hidden;">
        <div style="white-space:nowrap;animation:tickerScroll 30s linear infinite;font-size:10px;font-weight:900;color:#0a0a0a;letter-spacing:.08em;text-transform:uppercase;">
          &nbsp;&nbsp;&nbsp;&nbsp;
          ✂ Bienvenido a Fabiola Peluquería &amp; Estética &nbsp;·&nbsp;
          San Martín 443, Castro, Chiloé &nbsp;·&nbsp;
          Gracias por su preferencia &nbsp;·&nbsp;
          Sistema desarrollado por Tecno Jump &nbsp;·&nbsp;
          ✂ ${espera.length > 0 ? `Clientes en espera: ${espera.length}` : 'Sin clientes en espera'} &nbsp;·&nbsp;
          ✂ Bienvenido a Fabiola Peluquería &amp; Estética &nbsp;·&nbsp;
          San Martín 443, Castro, Chiloé &nbsp;·&nbsp;
          Gracias por su preferencia &nbsp;·&nbsp;
          Sistema desarrollado por Tecno Jump
  ...
  const _pub = {
    ...
    renderEstadisticas(periodo) {
      ...
      const chartLabel = document.getElementById('chartPeriodoLabel');
      if (chartLabel) chartLabel.textContent = labels[periodo] || '';
      ...
      // ── GRÁFICO DE VENTAS POR FECHA (Chart.js) ─────────────
      const ventasChartEl = document.getElementById('ventasChart');
      if (ventasChartEl && typeof Chart !== 'undefined') {
        // Agrupar ventas por fecha (día)
        const ventasPorFecha = {};
        ventas.forEach(v => {
          const fecha = new Date(v.timestamp).toLocaleDateString('es-CL', {
            day: '2-digit', month: '2-digit',
            ...(periodo === 'mes' ? { month: '2-digit' } : {})
          });
          ventasPorFecha[fecha] = (ventasPorFecha[fecha] || 0) + (v.price || 0);
        });
        ...
        const montos = fechasOrdenadas.map(f => ventasPorFecha[f]);

        // Destruir gráfico previo
        if (_ventasChart) { _ventasChart.destroy(); _ventasChart = null; }

        _ventasChart = new Chart(ventasChartEl, {
          type: 'line',
          data: {
            ...
            datasets: [{
              ...
              pointBorderWidth: 2,
              pointRadius: montos.length <= 7 ? 5 : 3,
              pointHoverRadius: 7,
              tension: 0.3,
            }]
          },
          options: {
            ...
            scales: {
              x: {
                ...
                ticks: {
                  color: '#a3a3a3',
                  font: { family: "'DM Sans', system-ui", size: 10, weight: '700' },
                  maxTicksLimit: 12,
                },
      ...
      document.getElementById('statBarberList').innerHTML = sortedBarb.length
        ? sortedBarb.map(([nombre, d], i) => {
            ...
            const av = bObj?.photo
              ? `<img src="${bObj.photo}" style="width:40px;height:40px;border-radius:10px;object-fit:cover;flex-shrink:0;">`
              : `<div style="width:40px;height:40px;border-radius:10px;background:#f5f5f5;display:flex;align-items:center;justify-content:center;font-weight:900;font-size:14px;color:#a3a3a3;flex-shrink:0;">${nombre.charAt(0)}</div>`;
            return `
              <div class="flex items-center gap-4 p-3 rounded-xl bg-app-bg">
                ${av}
                <div class="flex-1 min-w-0">
                  <div class="flex justify-between items-center mb-1">
                    <span class="font-black text-sm uppercase">${medals[i]||'  '} ${nombre}</span>
                    <span class="font-black text-teal text-sm">${fmt(d.total)}</span>
                  </div>
                  <div class="stat-bar-wrap">
                    <div class="stat-bar-fill ${i===0?'':'gold'}" style="width:${pct}%"></div>
                  </div>
                  <div class="text-[9px] text-muted font-bold mt-1">${d.cant} servicios · Ticket prom. ${fmt(d.cant ? Math.round(d.total/d.cant) : 0)}</div>
                </div>
              </div>`;
          }).join('')
        : '<p class="text-center text-neutral-300 text-[10px] font-black uppercase py-6">Sin datos en este período</p>';
      ...
      document.getElementById('statServicioList').innerHTML = sortedSrv.length
        ? sortedSrv.slice(0,8).map(([nombre, cnt], i) => {
            ...
            return `
              <div class="flex items-center gap-4">
                <div class="w-6 text-center font-black text-[10px] text-neutral-300">#${i+1}</div>
                <div class="flex-1 min-w-0">
                  <div class="flex justify-between items-center mb-1">
                    <span class="font-black text-xs uppercase truncate">${nombre}</span>
                    <span class="text-[10px] font-black text-neutral-600">${cnt}x · ${fmt(totalSrv)}</span>
                  </div>
                  <div class="stat-bar-wrap">
        ...
        : '<p class="text-center text-neutral-300 text-[10px] font-black uppercase py-4">Sin datos</p>';
      ...
      document.getElementById('statCierreList').innerHTML = cierresFiltrados.length
        ? cierresFiltrados.map(c => `
            <div class="p-4 border border-border rounded-xl bg-app-bg flex justify-between items-center">
              <div>
                <div class="font-black text-xs uppercase">${c.fecha}</div>
                <div class="text-[9px] text-muted font-bold mt-0.5">${(c.detalle||[]).filter(v=>!v.type).length} servicios</div>
              </div>
              <div class="text-right">
                <div class="font-black text-teal">${fmt(c.sis?.tot||0)}</div>
                <div class="text-[8px] text-muted font-bold uppercase">Sistema</div>
              </div>
            </div>`).join('')
        : '<p class="text-center text-neutral-300 text-[10px] font-black uppercase py-4">Sin cierres en este período</p>';
    },
